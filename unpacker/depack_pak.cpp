...
static u8 decodeStack[DECODE_SIZE];

/*--- Functions ---*/

static int pak_read_bits(MEM_STREAM *src, int num_bits)
{
	...
	while (mask > 0) {
		if (tmpMask == 0x80) {
			if (MemStreamRead(src, &srcByte, 1) <= 0)
			{
				srcByte = 0;
			}
...
{
	while (code > 255) {
		decodeStack[decodeStackOffset++] = (u8)tmpArray2[code].value;
		code = (unsigned long)tmpArray2[code].index;
	}
	decodeStack[decodeStackOffset] = (u8)code;

	return decodeStackOffset;
}

static void pak_write_dest(unsigned int value)
{
...
void pak_depack(MEM_STREAM *src, u8 **dstBufPtr, int *dstLength)
{
